...
451 条建议，每条写明花掉什么、换回什么、证据有多硬，来源只引期刊论文和官方文件。
...
[![条目](https://img.shields.io/badge/%E6%9D%A1%E7%9B%AE-451%20%E6%9D%A1-18794e?style=flat-square)](#目录)
[![证据分级](https://img.shields.io/badge/%E8%AF%81%E6%8D%AE%E5%88%86%E7%BA%A7-A%20290%20%C2%B7%20B%20112%20%C2%B7%20C%2049-915930?style=flat-square)](#证据分级)
[![原始文献](https://img.shields.io/badge/%E5%8E%9F%E5%A7%8B%E6%96%87%E7%8C%AE-865%20%E6%9D%A1%E9%93%BE%E6%8E%A5-565a5f?style=flat-square)](docs/核实记录/)
...
- **只想看结论最硬的**：在检索页里勾选证据等级 A，只留下有具体数字、来自荟萃分析或大型试验的 290 条。
- **只想看最值得做的**：勾选性价比「极高」，得到 80 条既不花钱、不花时间、不需要毅力，收益又落在最大一档的条目。再叠加一个「换回什么」，就是该口径下的优先清单。
...
全书 451 条中 A 级 290 条、B 级 112 条、C 级 49 条，另有 43 条标注了争议、36 处标注了 TODO 待核实。有争议的 A/B 级条目会标注「争议」并列出反方证据。所有来源只引原始文献（期刊论文附 DOI 或 PubMed 链接，或 WHO/CDC/国家统计局等官方机构报告），不引二手转述。不确定的数字标「待核实」。
...
全书 451 条中性价比极高 80 条（18%）、高 225 条（50%）、一般 146 条（32%）。中间一档偏厚是有意的：底层的收益量级只有三级判断，再往下切就是假装精度。
...
正文按节拆成 29 个文件放在 [book/](book/)，点上面目录里的节名进入。拆开是因为单文件已经超过 GitHub 渲染 Markdown 的 512 KB 上限，后面的节显示不出来；[在线检索页](https://eternity4719.github.io/HowToLiveBetter/)会把这些文件合起来读，用法不变。
