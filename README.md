...
1. [不要早死](#1-不要早死)：外因死亡、疫苗、筛查、心理危机。口径：ACM 或特定死因。
2. [不要慢慢死](#2-不要慢慢死)：烟酒、运动、睡眠、饮食、久坐。口径：ACM 或特定死因。
3. [不要浪费精力](#3-不要浪费精力)：睡眠、打断、多任务、决策疲劳、人际负债。口径：精力/时间。
4. [不要浪费时间](#4-不要浪费时间)：无收益项目、沉没成本、拖延、会议、通勤。口径：时间。
5. [不要浪费钱](#5-不要浪费钱)：订阅、彩票、利息、保险、基金费率。口径：金钱。
...
- 收益：荟萃分析（123 项试验，61 万余人）：收缩压每降 10 mmHg，主要心血管事件 RR 0.80（95% CI 0.77 到 0.83），卒中 RR 0.73，心力衰竭 RR 0.72，ACM 下降 13%（RR 0.87，95% CI 0.84 到 0.91）。中国 35 到 75 岁 170 万人筛查：44.7% 有高血压，其中知晓 44.7%、治疗 30.1%、控制 7.2%
- 证据等级：A
...
- 收益：心肌梗死后随机双盲试验（2571 人）：接种流感疫苗 12 个月 ACM 2.9% 对安慰剂 4.9%（HR 0.59，95% CI 0.39 到 0.89），心血管死亡 HR 0.59。荟萃分析：流感疫苗使主要心血管事件 3.6% 对 5.4%（RR 0.66，95% CI 0.53 到 0.83），心血管死亡 RR 0.74（95% CI 0.42 到 1.30，无统计学意义）
- 证据等级：A
...
- 收益：美国 NLST 随机试验（53,454 人，55 到 74 岁、吸烟史 30 包年以上、戒烟不超过 15 年）：低剂量 CT 比胸片使肺癌死亡相对下降 20.0%（95% CI 6.8 到 26.7），ACM 相对下降 6.7%（95% CI 1.2 到 13.6）
- 证据等级：A
...
本节只收对全因死亡率（ACM）效应大、证据硬的慢性风险因素，按性价比从高到低排。除特别标注的随机试验外，数字都来自观察性研究，HR/RR 里含混杂和反向因果，读成「方向和量级」而不是「照做就能拿到的折扣」。

### 1. 戒烟，越早越好
<!-- 成本标签: 钱=0 时间=少 毅力=是 -->
- 成本：戒断期的毅力（数周到数月）；金钱是负成本，一天一包烟约 20–30 元，戒了就省下。
- 收益：美国队列（观察性）：现吸烟者比从不吸烟者预期寿命短 10 年以上；40 岁前戒掉可消除继续吸烟所带来死亡风险的约 90%，25–34、35–44、45–54 岁戒烟分别约多活 10、9、6 年（原文以寿命年报告）。中国队列（观察性）：2010 年代城市男性吸烟者 ACM RR 1.65，农村男性 RR 1.22；主动戒烟满 10 年后归因风险接近消失。
- 证据等级：A
...
<!-- 成本标签: 钱=0 时间=少 毅力=些 -->
- 成本：零；换成白水或无糖茶还省钱。
- 收益：美国两大队列（观察性，约 11.8 万人、3.6 万例死亡）：每天 ≥2 份 vs 每月 <1 份，ACM HR 1.21；每天 1–2 份 HR 1.14。欧洲 EPIC 队列（观察性，45 万人）：每天 ≥2 杯 vs 每月 <1 杯，含糖软饮 HR 1.08，所有软饮合计 HR 1.17。
- 证据等级：A
...
- 收益：15 个队列的荟萃分析（观察性，47471 人、3013 例死亡）：按步数四分位，中位 5801、7842、10901 步/天 vs 3553 步/天，ACM HR 分别 0.60、0.55、0.47；≥60 岁者获益在 6000–8000 步后趋平，<60 岁者在 8000–10000 步后趋平。另一荟萃分析：从约 3867 步/天起开始有效，每多 1000 步/天 ACM 降约 15%。
- 证据等级：A
...
- 收益：随机试验荟萃分析：收缩压每降 10 mmHg，ACM RR 0.87，主要心血管事件 RR 0.80，卒中 RR 0.73，心衰 RR 0.72；他汀使 LDL 每降 1.0 mmol/L，ACM RR 0.90，主要血管事件 RR 0.78。依从性荟萃分析（观察性）：他汀、降压药依从 ≥80% 者 vs 依从差者，ACM RR 分别 0.55、0.71。
- 证据等级：A
...
- 备注：只对有用药指征的人成立，健康人不需要吃。依从性那组数字是观察性的，「能坚持吃药的人本身更自律」会夸大效应，前两组随机试验数字更可靠。本节不单列血糖控制：强化降糖对 ACM 的证据不如血压、血脂一致。

### 6. 每晚睡 7 小时左右，作息固定
<!-- 成本标签: 钱=0 时间=多 毅力=些 -->
- 成本：时间（多数人是把刷手机的时间换成睡觉）；固定作息需要一点自律。
- 收益：荟萃分析（观察性，16 项研究、138 万人、11.3 万例死亡）：睡得短者 ACM RR 1.12，睡得长者 RR 1.30。剂量反应荟萃分析：以 7 小时为最低点，少于 7 小时每少 1 小时 RR 1.06，多于 7 小时每多 1 小时 RR 1.13。UK Biobank 约 6.1 万人腕表数据（观察性）：睡眠最规律的四个五分位 vs 最不规律的五分位，ACM 低 20%–48%，规律性比时长更能预测死亡。
- 证据等级：A
...
<!-- 成本标签: 钱=0 时间=多 毅力=些 -->
- 成本：每天 20–45 分钟；不花钱。
- 收益：多队列汇总分析（观察性）：达到指南下限 1–2 倍（7.5–15 MET·h/周，约合快走 150–300 分钟/周）者 vs 不运动者 ACM HR 0.69；不足下限也有 HR 0.80；3–5 倍时 HR 0.61 到顶，再多不再降但也无害（≥10 倍 HR 0.69）。加速度计测量的荟萃分析：中高强度活动最高四分位 vs 最低 HR 0.52。
- 证据等级：A
...
- 收益：队列荟萃分析（观察性）：做力量训练者 vs 不做者 ACM 低 10%–17%；剂量反应呈 J 型，约 30–60 分钟/周时降幅最大（约 10%–20%），更多不再增益；力量+有氧都做 vs 都不做，ACM 更低。
- 证据等级：A
...
<!-- 成本标签: 钱=0 时间=少 毅力=些 -->
- 成本：零；设个提醒。
- 收益：美国队列（观察性，加速度计）：久坐总时长最高四分位 vs 最低 ACM HR 2.63；连续不起身的久坐时段最长四分位 vs 最短 HR 1.96，总量和连续时长各自独立相关。百万人汇总分析（观察性）：每天坐 >8 小时且几乎不运动者 vs 坐 <4 小时且最活跃者 HR 1.59；每天约 60–75 分钟中等强度活动可抵消久坐的超额风险（最活跃组坐 >8 小时 HR 1.04，不显著）。
- 证据等级：A
...
<!-- 成本标签: 钱=0 时间=少 毅力=些 -->
- 成本：零，甚至省钱；口味上的一点牺牲。
- 收益：荟萃分析（观察性）：加工肉最高 vs 最低摄入组 ACM RR 1.23，总红肉 RR 1.29，未加工红肉 RR 1.10（不显著）；另一荟萃分析按剂量：加工肉每多 1 份/天 RR 1.23，红肉每多 1 份/天 RR 1.10。
- 证据等级：A
...
<!-- 成本标签: 钱=0 时间=少 毅力=些 -->
- 成本：社交场合的一点尴尬；省钱。
- 收益：83 项前瞻研究 60 万现饮者汇总（观察性）：ACM 最低点在每周 ≤100 g 纯酒精（约合 5% 啤酒 2.5 L 或 40° 白酒 300 mL）；40 岁时每周 100–200 g 者预期寿命少约 6 个月，200–350 g 少 1–2 年，>350 g 少 4–5 年（原文以寿命年报告）。GBD 2016：把各类健康损失合计，风险最低的饮酒量是 0 杯/周。校正研究设计偏倚的荟萃分析：少量饮酒（1.3–24 g/天）vs 终身不饮 RR 0.93（不显著），45–64 g/天 RR 1.19，≥65 g/天 RR 1.35。
- 证据等级：A
...
- 备注：争议。J 型曲线一方：34 项前瞻研究荟萃（Di Castelnuovo A 等 (2006). Alcohol dosing and total mortality in men and women: an updated meta-analysis of 34 prospective studies. Archives of Internal Medicine. https://doi.org/10.1001/archinte.166.22.2437）报告少量饮酒 ACM 最多低 17%–18%，男性 ≤4 杯/天、女性 ≤2 杯/天仍呈负相关。反方认为「不饮者」里混入了因病戒酒者和整体健康更差的人，Zhao 2023 校正这些偏倚后保护作用消失，GBD 2018 也给 0 最安全。稳妥读法：少喝不会明显伤寿命，多喝一定伤，「为了健康开始喝」没有依据。

### 12. 把一部分精米白面换成全谷物
<!-- 成本标签: 钱=少 时间=少 毅力=些 -->
- 成本：糙米、燕麦、全麦面价格略高，口感需要适应。
- 收益：荟萃分析（观察性）：全谷物每多 90 g/天（约 3 份）ACM RR 0.83，收益持续到 210–225 g/天；另一荟萃分析每多 1 份/天 RR 0.92。
- 证据等级：A
...
<!-- 成本标签: 钱=少 时间=中 毅力=些 -->
- 成本：每天几元到十几元；洗切的时间。
- 收益：荟萃分析（观察性）：每多 200 g/天 ACM RR 0.90，收益持续到 800 g/天；美国两大队列加 26 项队列荟萃：每天 5 份 vs 2 份 ACM HR 0.87，2 份水果+3 份蔬菜最优，再多不再降。
- 证据等级：A
...
- 收益：伞状综述（汇总观察性荟萃分析）：超加工食品摄入高 vs 低，ACM RR 1.21，心血管病死亡 RR 1.50；两者证据分级分别为「高度提示」和「令人信服」，但 GRADE 确定性为低/极低。
- 证据等级：A
...
- 收益：中国慢性病前瞻队列（观察性，27.1 万无心血管病成人）：做饭用固体燃料 vs 清洁燃料 ACM HR 1.11，取暖用固体燃料 HR 1.14；已从固体燃料换成清洁燃料者 vs 继续用固体燃料者，做饭 HR 0.87、取暖 HR 0.67。室外 PM2.5：104 项队列荟萃，长期暴露每高 10 µg/m³，自然原因死亡 RR 1.08。
- 证据等级：A
...
- 收益：239 项前瞻研究个体数据荟萃（观察性，限定从不吸烟、基线无慢性病、存活满 5 年者）：ACM 在 BMI 20–25 最低，25–27.5 HR 1.07，27.5–30 HR 1.20，30–35 HR 1.45，35–40 HR 1.94，40–60 HR 2.76；BMI 25 以上每高 5 kg/m²，东亚人群 HR 1.39。
- 证据等级：A
...
- 证据等级：C
- 来源：中国证监会 (2023). 证监会有关部门负责人就公募基金费率改革答记者问. http://www.csrc.gov.cn/csrc/c100028/c7418692/content.shtml；Fama EF, French KR (2010). 见第 8 条. https://doi.org/10.1111/j.1540-6261.2010.01598.x
- 备注：复利算式为纯算术，假设两只基金费前收益相同；实际还要算申购费、赎回费和 C 类份额的销售服务费。不构成投资建议。

### 10. 健身房按次付费或短周期付费，除非有一年以上的稳定记录
<!-- 成本标签: 钱=少 时间=少 毅力=否 -->
...
### 11. 大额非必需购买设 24 小时冷静期，网购善用七日无理由退货
<!-- 成本标签: 钱=0 时间=少 毅力=些 -->
- 成本：延后一天满足；退货运费通常自付
...
### 2. 被欠薪就申请劳动仲裁，不收费；打不起官司申请法律援助
<!-- 成本标签: 钱=0 时间=中 毅力=些 -->
- 成本：0 元仲裁费；准备劳动合同、工资条、考勤、聊天记录等证据，跑仲裁委 1 到 3 趟；申请时效 1 年，从知道权利被侵害起算
- 收益：劳动争议仲裁不收费，经费由财政保障；受理后 45 日内结案，复杂案件最多延 15 日；请求支付劳动报酬或确认劳动关系的，经济困难者可申请无偿法律援助，进城务工人员申请支付劳动报酬的免予核查经济困难；被欠薪的农民工有权投诉，任何人可向人社部门举报，涉嫌拒不支付劳动报酬罪的移送公安（全国）
- 证据等级：A
- 来源：全国人大常委会 (2007). 劳动争议调解仲裁法（主席令第八十号，第二十七、四十三、五十三条）. https://chinajob.mohrss.gov.cn/h5/c/2022-07-15/356212.shtml；国务院 (2019). 保障农民工工资支付条例（国务院令第 724 号，第十、四十一条）. https://www.gov.cn/gongbao/content/2020/content_5469641.htm；全国人大常委会 (2021). 法律援助法（第二、三十一、四十二条）. https://www.beijing.gov.cn/zhengce/zhengcefagui/qtwj/202504/t20250402_4053713.html
- 备注：先拨 12333 找人社部门或直接向劳动监察投诉，通常比仲裁快；仲裁要先证明劳动关系，入职起就留好证据。法律援助法全文引自北京市政府网转载页，司法部与人大网页面未能打开；公共法律服务热线 12348 可先咨询，该号码未在本节核实的原文中出现。

### 3. 走投无路时去救助站，管吃住和返乡车票
<!-- 成本标签: 钱=0 时间=少 毅力=些 -->
...
### 10. 办失业登记后争取就业困难人员认定，拿社保补贴或公益性岗位
<!-- 成本标签: 钱=0 时间=中 毅力=些 -->
...
### 12. 不交押金、不押证件、不签「培训贷」、不进传销、不借高利贷
<!-- 成本标签: 钱=0 时间=少 毅力=些 -->
...
### 14. 把住和吃压到最低：合租月租代替日租，自炊和助餐食堂代替外卖
<!-- 成本标签: 钱=0 时间=中 毅力=些 -->
...
### 16. 实在没地方睡，24 小时营业场所只能应急一晚，优先去救助站
<!-- 成本标签: 钱=少 时间=少 毅力=些 -->
